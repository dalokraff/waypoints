...
local name_to_career_dict = require("scripts/mods/Waypoints/name_career_dict")
local atlas = require("materials/Waypoints/store_copy")
...
mod.waypoints = {
	...
	['kruber_questingknight'] = {
		name = "kruber_questingknight",
		waypoint_is_set = false,
		waypoint_should_follow = false,
		unit_to_follow = nil,
		waypoint_current_time = 0,
	...
	['kerillian_waystalker'] = {
		name = "kerillian_waystalker",
		waypoint_is_set = false,
		waypoint_should_follow = false,
		unit_to_follow = nil,
		waypoint_current_time = 0,
	...
	['kerillian_handmaiden'] = {
		name = "kerillian_handmaiden",
		waypoint_is_set = false,
		waypoint_should_follow = false,
		unit_to_follow = nil,
		waypoint_current_time = 0,
	...
	['kerillian_thornsister'] = {
		name = "kerillian_thornsister",
		waypoint_is_set = false,
		waypoint_should_follow = false,
		unit_to_follow = nil,
		waypoint_current_time = 0,
	...
	--using this as a place holder for WP icon
					...
					if charwp.waypoint_is_set then
						...
						if charwp.waypoint_current_time < mod.waypoint_lifespan_in_seconds then
							...
							if player.player_unit then
								...
								local career_name = name_to_career_dict[charwp.name]
								local icon_name = "store_tag_icon_"..career_name

								local texture_settings = atlas[icon_name]
								...
								if is_clamped or is_behind then
									if not waypoint_on_me then
											...
											Gui.bitmap_uv(mod.waypoint_gui, "store_copy", uv00, uv11, Vector2(waypoint_position2d[1], waypoint_position2d[2]), Vector2(waypoint_size, waypoint_size))
										end
									else
...
mod.create_gui = function(self)
	if Managers.world:world("top_ingame_view") then
		...
		mod.waypoint_gui = World.create_screen_gui(top_world, "immediate",
			...
			"material", "materials/Waypoints/store_copy"
			)
	end
end
